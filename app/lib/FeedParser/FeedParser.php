...
namespace Feeds2Social\lib\FeedParser;

 /**************************************************************************
 * Copyright (C) 2011 Alex Dzyoba <finger@reduct.ru>
 * Modified (C) 2018 Demiurge Ash
 *
 ...
 *
 * For RSS branch I implement only one class RSS2, because RSS
 * 0.91, 0.92 and 2.0 all have same tags and no XML namespace.
 *
 * For RDF branch I implement RDF class, that handles all that crappy
	...
	function __construct($xml)
	{
		...
		switch (true) 
		{
			...
			case ($doc_element->namespaceURI == 'http://purl.org/rss/1.0/' || 
				 ...
				 $this->feed = new RDF($this->model, '1.0');
				 break;
			...
			case ($doc_element->namespaceURI == 'http://purl.org/rss/1.1/' || 
				...
				$this->feed = new RDF($this->model, '1.1');
				break;
			...
			case ($doc_element->namespaceURI ==	'http://my.netscape.com/rdf/simple/0.9/' ||
				...
				$this->feed = new RDF($this->model, '0.90');
				break;
